 ...
 * (ARCHITECTURE §11). Claims the item on mount; the trainer corrects the LLM draft into the
...
function ReviewItem({ uploadId }: { uploadId: string }) {
  ...
  const progress = useQueueProgress(uploadId, showProgress);
      ...
      <div className="mb-3 flex flex-wrap items-baseline gap-x-3">
        ...
        {claimConflict && (
          <span className="rounded bg-amber-tint px-2 py-0.5 text-xs font-medium text-amber">
            Wird gerade von jemand anderem geprüft — nur Ansicht
          </span>
        )}
      </div>

      {/* Learner progress — grading context for every trainer (known-trainer model, §H1.3). */}
      <div className="mb-4">
