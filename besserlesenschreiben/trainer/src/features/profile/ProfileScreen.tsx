...
const ROLE_LABEL = { trainer: 'Trainer:in', admin: 'Administrator:in' } as const;

/**
 * The trainer's OWN profile: display name (editable — it's what colleagues see in the top bar),
...
export function ProfileScreen() {
  ...
  if (!trainer) return <p className="py-16 text-center text-ink-soft">Lädt …</p>;

  const startEdit = () => {
    setDraft(trainer.name);
    rename.reset();
    setEditing(true);
  };
  const save = () => {
    const name = draft.trim();
    if (!name || name === trainer.name) return setEditing(false);
    rename.mutate(name);
  };
      ...
      <div className="divide-y divide-line rounded-card bg-surface shadow-sm ring-1 ring-line">
        <Row label="Anzeigename">
          {editing ? (
            ...
            <div className="flex items-center gap-1.5">
              <span className="font-medium text-ink">{trainer.name}</span>
        ...
        <Row label="Anmelde-E-Mail">
          <span className="text-ink">{trainer.email}</span>
        </Row>

        <Row label="Rolle">
          <span className="rounded-full bg-teal-tint px-2.5 py-1 text-xs font-semibold text-teal-dark">
            {ROLE_LABEL[trainer.role]}
          </span>
        </Row>

        <Row label="Zugang seit">
          <span className="text-ink">
            {new Date(trainer.createdAt).toLocaleDateString('de-AT', { day: 'numeric', month: 'long', year: 'numeric' })}
          </span>
        </Row>
      </div>
