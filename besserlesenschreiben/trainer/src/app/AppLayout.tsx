...
/** Portal chrome: a slim top bar (brand + nav + trainer identity + logout) over the routed Outlet.
 *  Desktop/tablet, full-width content (ARCHITECTURE §1a/§11). */
export function AppLayout() {
  const { trainer, logout } = useStaffAuth();
      ...
      <header className="sticky top-0 z-10 border-b border-line bg-surface/90 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-screen-xl items-center justify-between px-5">
          <div className="flex items-center gap-5">
            {/* Brand mark + the signed-in trainer, mirroring the family app's "(b) name". */}
