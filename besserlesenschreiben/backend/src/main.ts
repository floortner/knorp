...
async function bootstrap(): Promise<void> {
  ...
  if (process.env.NODE_ENV === 'production' && allowedOrigins.length === 0) {
    throw new Error('CORS allowlist is empty: set WEB_ORIGIN (and TRAINER_ORIGIN) in production (ARCHITECTURE §4).');
  }
  ...
  // Swagger UI + the raw OpenAPI JSON are dev-only surface — don't serve them in production (the contract
  // is committed to the repo and regenerated by scripts/export-openapi.ts, so nothing depends on the
  // runtime route). Keeps free recon off the prod box (security review P3). `isProd` is defined above.
  if (!isProd) {
